...
TABLES_DICT = {"+": "addition", "-": "subtraction", "x": "multiplication", "mix": "mix"}
...
class WorkSheetGenerator:
    def __init__(
        self, type_: str, tables: bool, num_questions=90, start_num=1000, end_num=3000
    ):
    ...
    def randomize_question(self):
        ...
        self.final_questions_list = sample(
            self.final_questions_list, self.num_questions
        )
    ...
    def print_final_worksheet(self, number_of_columns=9):
        LOGGER.info(
            f"{TABLES_DICT[self.main_type].capitalize()} Worksheet"
        ...
        pdf = weasyprint.HTML(filename=final_file_path).write_pdf(
...
@click.option(
    ...
    multiple=False,
    prompt="What type of calculation?",
...
@click.option(
    ...
    is_flag=False,
    default=99,
    prompt="Starting number for questions",
...
@click.option(
    ...
    is_flag=False,
    default=999,
    prompt="Ending number for questions: ",
