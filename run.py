...
from worksheet_generator import LOGGER
...
class MathWorksheetGenerator:
    ...
    def __init__(self, type_: str, max_number: int, total_questions: int):
        ...
        self.num_x_cell = 6
        self.num_y_cell = 3
        self.total_question = total_questions
        try:
            assert self.total_question % 40 == 0, 'Must be a multiple of 40'
        except AssertionError as _:
            self.total_question -= (self.total_question % 40)
        self.font_1 = 'Times'
        self.font_2 = 'Arial'
    ...
    def get_list_of_questions(self) -> List[QuestionInfo]:
        ...
        Initially trying for unique questions, but allowing duplicates if needed
    ...
    def print_question_row(self, data: List[QuestionInfo], offset: int):
        ...
        for _ in range(self.num_x_cell):
            ...
            self.print_edge_vertical_separator()
        self.pdf.ln()
        self.pdf.output(f"data/worksheet_{arrow.now()}.pdf")
...
@click.option('--digits', default='2',
              type=click.Choice(['1', '2', '3']),
              help='range of numbers: 1: 0-9, 2: 0-99, 3: 0-999'
                   '(default: 2 -> 0-99)')
@click.help_option('-h', '--help')
def main(type, digits, questions):
    """Generate Maths Addition/Subtraction/Multiplication Exercise Worksheet"""
    ...
    if digits == "1":
        ...
        size_ = 99
    new_pdf = MathWorksheetGenerator(type, size_, questions)
    ...
    new_pdf.pdf.output("data/worksheet.pdf")
