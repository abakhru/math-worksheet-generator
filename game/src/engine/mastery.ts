...
export function isTableUnlocked(table: number, progress: Record<number, any>): boolean {
  ...
  return prevProgress?.stars >= 2
}

// Get all unlocked tables
export function getUnlockedTables(progress: Record<number, any>): number[] {
  return ALL_TABLES.filter((t) => isTableUnlocked(t, progress))
}

// Get next table to unlock
export function getNextTableToUnlock(progress: Record<number, any>): number | null {
